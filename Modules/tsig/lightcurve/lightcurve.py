...
def create_random_lightcurve(options=('trapezoid', 'sinusoid'),
...
def create_cartoon_lightcurve(options=('trapezoid', 'sinusoid'),
                              extreme=False,
                              prng=np.random):
    ...
    raise ValueError("No lightcurve type specified.")
...
class LightCurve(dict):
    ...
    def get_flux(self, t):
        """
        Return an array of flux values for the corresponding times.
        """
...
class ConstantLightCurve(LightCurve):
    ...
    def get_mag(self, t):
        return np.zeros_like(t, dtype=float) + self['offset']

    def integrated(self, t, exptime=30.0 / 60.0 / 24.0, resolution=100):
        """constant light curve is a degenerate case"""
        return self.get_mag(LightCurve._values_like(t))
...
class TrapezoidLightCurve(LightCurve):
    ...
    def time_from_mid_transit(self, t):
    ...
    def get_mag(self, t):
        ...
        if flux.shape:
            dt = np.abs(self.time_from_mid_transit(t))
            ...
            if dt <= start:
                flux = depth
            elif start < dt <= finish:
                flux = (finish - dt) / (finish - start) * depth
        return -flux
...
class FilebasedLightCurve(LightCurve):
    ...
    def get_mag(self, t):
        ...
        minutetime = [(time[x]-time[0])*24*60 for x in range(len(time))]
        try:
...
class ASCIILightCurve(FilebasedLightCurve):
    def __init__(self, cfgfile='example.cfg', name=''):
        ...
        try: 
            ...
            logger.debug("lcformat %s", str(self.cols))
        except configparser.NoOptionError:
            self.cols = {'jd': 2, 'rlc': 8, 'ltflc': 12,
                        'x': 3, 'y': 4, "bg": 12, "bgerr": 12, 'cadence': 2}
            logger.warning("Use default LC format, type is %s", str(self.cols))  
    ...
    def load_from_file(self, label='all'):
        ...
        if headerline.startswith('#'):
            newdict = dict(list(zip(headerline.split()[1:],np.arange(len(headerline.split())-1)+1)))
            self.cols.update(newdict)
            logger.debug("Using columns from file: %s", str(self.cols))
        else: 
            logger.debug("File has no header line, using default format or format from configuration file: %s", str(self.cols))
        ...
        if label == 'all':
            ...
            try:
                ...
                logger.warning("ltflc for %s not created yet, "
                                    "you might want to detrend first",
                                    self.name)
            try:
                ...
                logger.warning("coordinate columns x, "
            ...
            try:
                ...
                logger.warning("Warning: background columns bg, bgerr "
    ...
    def write_to_file(self, outfile, replace=True, outheader="# jd rlc"):
        ...
        if os.path.exists(outfile) and (not replace):
            logger.warning("write aborted: file exists at %s", outfile)
            return
        logger.info("outfile header is %s", outheader)
        with open(outfile, mode='w') as fout:
            fout.write(outheader)
            fout.write('\n')        
            for i in range(len(self.data['jd'])):
...
class HDFLightCurve(FilebasedLightCurve):
    ...
    def __init__(self,name=''):
        ...
        logger = logging.getLogger(HDFLightCurve.__name__)
...
class FITSLightCurve(FilebasedLightCurve):
    ...
    def __init__(self, name='', cfgfile='example.cfg'):
        ...
        logger = logging.getLogger(FITSLightCurve.__name__)
        logger.addHandler(logging.NullHandler())
    ...
    def load_from_file(self, label='all'):
        ...
        with fits.open(self.name) as hdulist:
            ...
            logger.debug("lenth of data is %d", len(self.data['jd']))
