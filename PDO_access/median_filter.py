...
def median_filter(x, y, num_bins, bin_width=None, x_min=None, x_max=None,
  ...
  Args:
    ...
    return_error: If true, returns the error on the median in each bin as well as the medians themselves

  Returns:
    1D NumPy array of size num_bins containing the median y-values of uniformly
    spaced bins on the x-axis.

    (If return_error): 1D NP array of size num_bins containing the standard
      errors of the median y_values of uniformly spaced bins on the x-axis.
  ...
  errors = np.repeat(np.nan, num_bins)
